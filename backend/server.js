...
    });
    ...
    // === START OF FIX ===
    // Dynamically import the ESM module 'espnService.js'
    // The error log implies the file is named espnService.js
    const { getTeamMatchupStats } = await import('./espnService.js');
    // === END OF FIX ===
...
      // Assuming matchupData is the raw response from the AI
      // You may need to adjust this logging
      sport: sport.toUpperCase(),
      teams: `${team_1} vs ${team_2}`
    });

    // Assuming the AI response is the data you want to send
...
B   const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) {
