...
import type { MLBProjectionInput, MLBTeamInput, WindDirection } from './mlb.js';
...
function n(v: string | undefined): number | undefined {
  ...
  if(!Number.isFinite(parsed)) throw new Error('Invalid numeric MLB CSV cell');
  return parsed;
}
...
async function fetchCsv(file: string, required: boolean, readText: (file: string) => Promise<string>): Promise<Record<string, string>[]> {
  try {
...
export async function loadMLBStatsFrom(readText: (file: string) => Promise<string>): Promise<MLBStats> {
  const [offenseRows, bullpenRows, parkRows, pitcherRows, slateRows, meta] = await Promise.all([
    fetchCsv('mlb_team_offense.csv', true, readText),
    fetchCsv('mlb_bullpen.csv', false, readText),
    fetchCsv('mlb_parks.csv', false, readText),
    fetchCsv('mlb_starters.csv', false, readText),
    fetchCsv('mlb_slate.csv', false, readText),
    readText('last_updated.json').then(JSON.parse)
      .catch(() => null),
  ]);

  if(!offenseRows.length) throw new Error('MLB offense CSV is empty');
  const identifiers=new Set<string>();
  for(const row of offenseRows) {
    if(!row.team || !row.abbreviation || !['wrc_plus','woba','ops','runs_per_game'].some(key=>key in row)) throw new Error('MLB offense schema is invalid');
    const id=row.abbreviation.toUpperCase();
    if(identifiers.has(id)) throw new Error('Duplicate MLB team '+id);
    identifiers.add(id);
  }
...
export function loadMLBStats(): Promise<MLBStats> {
  if (!cache || Date.now() - cachedAt > 60000) {
    cachedAt=Date.now();
    cache = loadMLBStatsFrom(async file => {
